...
def frame2(files):
    ...
    for thing in range(len(files)):
        Label(second_frame, text=f'{thing+1}. {doc_files[thing]}', font='Helvetica 12 bold').grid(row=thing, column=0, pady=3, sticky=W)
...
def merge(ext):
    ...
    if len(dest_bar.get())!=0:
        if doc_files:
            if(ext=='.pdf'):
                ...
                print(doc_paths[0])
                path_d= dest_bar.get().replace(str('\\'), "/") + "/" +"mergedfilesoutput.docx"
                result = Document(doc_paths[0])
                result.add_page_break()
                composer = Composer(result)
                for i in range(1, len(doc_paths)):
                    doc = Document(doc_paths[i])
                    if i != len(doc_paths) - 1:
                        doc.add_page_break()
                    composer.append(doc)
                composer.save(path_d)


            elif(ext=='.txt'):
                path_d= dest_bar.get().replace(str('\\'), "/") + "/" +"mergedfilesoutput.txt"
                f1=open(path_d, "wb")
                for i in doc_paths:
                    f=open(i, "rb")
                    f1.write(f.read())
    ...
    open = Button(frame_bottom, width=10, bg='bisque', padx=15, pady=20, text='Open file',
                  command=lambda: open_file(path_d))
...
search_bar.insert(0, "Enter search directory Path:")
...
dest_bar.insert(0, "Enter destination path:")
